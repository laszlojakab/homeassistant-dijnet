...
from .controller import (DijnetController, Invoice, InvoiceIssuer, PaidInvoice,
                         get_controller)
...
class DijnetPaymentCalendarEventDevice(CalendarEntity):
    ...
    def device_info(self) -> DeviceInfo:
        return DeviceInfo(
            ...
            identifiers={(DOMAIN, self._invoice_issuer.issuer +
                          '|' + self._invoice_issuer.issuer_id)},
    ...
    async def _update_invoices(self):
        self._unpaid_invoices = [
            unpaid_invoice for
            unpaid_invoice in
            await self._controller.get_unpaid_invoices() if
            unpaid_invoice.display_name == self._invoice_issuer.display_name and
            unpaid_invoice.provider == self._provider
        ]
        self._paid_invoices = [
            paid_invoice for
            paid_invoice in
            await self._controller.get_paid_invoices() if
            paid_invoice.display_name == self._invoice_issuer.display_name and
            paid_invoice.provider == self._provider
        ]
    ...
    def _get_event(self, invoice: Invoice) -> CalendarEvent:
        ...
        return CalendarEvent(
            ...
            f'{invoice.provider} befizetés, összeg: {str(invoice.amount)}, határidő: {invoice.deadline.strftime("%Y.%m.%d")}',
            'https://dijnet.hu/'
        )
...
async def async_setup_entry(
    ...
    async_add_entities([
        DijnetPaymentCalendarEventDevice(controller, config_entry.entry_id, issuer, provider) for
        issuer in await controller.get_issuers() for
        provider in issuer.providers
    ])
