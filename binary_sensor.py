...
DEFAULT_NAME = "Dijnet Binary Sensor"
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    """Set up the Dijnet binary sensor."""
    username = config[CONF_USERNAME]
    password = config[CONF_PASSWORD]
    async_add_entities([DijnetSensor(username, password)], True)


class DijnetSensor(BinarySensorDevice):
    # """Representation of a Dijnet binary sensor."""

    def __init__(self, username, password):
        """Initialize the Dijnet binary sensor."""
        self._state = None
    ...
    def name(self):
        ...
        return 'Dijnet fizetendő számla'
    ...
    def update(self):
        ...
        self._state = "on" if len(unpaidInvoicesPq) > 0 else "off"
